...
class Azure:
    ...
    def __init__(self):
        ...
        self.KEY, self.BASE_URL, self.largePersonGroupId = self.get_secrets(self.ROOT_PATH)
        CF.Key.set(self.KEY)
    ...
    def add_face(self, imageBytes, person_id):
        ...
        if len(responseDetection): #verify face detect
            attributes = responseDetection[0]
            targetFace = self.get_face_rectangle(responseDetection[0])
            imgObject = Img(imageBytes)
            if person_id:
                try:
                    CF.large_person_group_person_face.add(
                        imgObject, self.largePersonGroupId, person_id, target_face=targetFace)
                    print("Added face!!")
                except requests.exceptions.ConnectionError:
    ...
    def create_person(self, name):
        ...
        try:
            print("person creating try")
            person_id = CF.large_person_group_person.create(self.largePersonGroupId,name)
            print("ID",person_id)
            print("The person {} was created with ID: {}".format(name, person_id))
        except requests.exceptions.ConnectionError:
            print("person creating try failed")
            self.codeError = self.INTERNET_ERROR
        except:
    ...
    def identify(self, imageBytes, threshold=None):
        ...
        if len(responseDetection) > 0:
            ...
            try:
                responseIdentify = CF.face.identify(
                    [faceId], large_person_group_id=self.largePersonGroupId, threshold=threshold,)
